    ...
    for (i = 0; i < 6; i++)
    {
        ...
        if (moveFlags & flags)
        {
            ...
            for (j = 0; j < 4; j++)
            {
                ...
                moveFlags = AI_TypeCalc(move, type1, type2, gBattleMons[battlerIn1].ability);
...
static bool8 ShouldUseItem(void)
{
    ...
    for (i = 0; i < 4; i++)
    {
        ...
        ewram160D8(gActiveBattler) = GetAI_ItemType(item, medicineGroup);
        ...
        case AI_ITEM_CURE_CONDITION:
            ...
            if (medicineGroup == MEDICINE_GROUP_STATUS_RESTORE && gBattleMons[gActiveBattler].status1 & STATUS_SLEEP)
            {
               ewram160DA(gActiveBattler) |= 0x20;
                shouldUse = TRUE;
            }
            if (medicineGroup == MEDICINE_GROUP_STATUS_RESTORE && (gBattleMons[gActiveBattler].status1 & STATUS_POISON || gBattleMons[gActiveBattler].status1 & STATUS_TOXIC_POISON))
            {
               ewram160DA(gActiveBattler) |= 0x10;
                shouldUse = TRUE;
            }
            if (medicineGroup == MEDICINE_GROUP_STATUS_RESTORE && gBattleMons[gActiveBattler].status1 & STATUS_BURN)
            {
               ewram160DA(gActiveBattler) |= 0x8;
                shouldUse = TRUE;
            }
            if (medicineGroup == MEDICINE_GROUP_STATUS_RESTORE && gBattleMons[gActiveBattler].status1 & STATUS_FREEZE)
            {
               ewram160DA(gActiveBattler) |= 0x4;
                shouldUse = TRUE;
            }
            if (medicineGroup == MEDICINE_GROUP_STATUS_RESTORE && gBattleMons[gActiveBattler].status1 & STATUS_PARALYSIS)
            {
